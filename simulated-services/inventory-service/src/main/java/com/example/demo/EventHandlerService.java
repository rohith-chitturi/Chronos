...
public class EventHandlerService {
    ...
    public void consumeFaultRule(String message) {
        try {
            ...
            if ("inventory-service".equals(rule.getTargetService())) {
    ...
    public void consumeReplayCommand(String message) {
        try {
            ...
            String incomingTopicToSimulate = "payment-events";
    ...
    public void consume(String message) {
        try {
            ...
            String myEventType = "INVENTORY_RESERVED";
            if ("inventory-service".equals("inventory-service") && "inventory-timeout".equals(failAt)) {
                myEventType = "INVENTORY_TIMEOUT";
            }
            if ("inventory-service".equals("payment-service") && "payment-failed".equals(failAt)) {
            ...
            if (incomingEvent.contains("FAILED") || incomingEvent.contains("TIMEOUT")) {
                if ("inventory-service".equals("order-service")) {
                    myEventType = "ORDER_FAILED";
                } else {
                    return; 
                }
            }
            if (incomingEvent.equals("SHIPPING_CREATED") && "inventory-service".equals("order-service")) {
            ...
            if ("inventory-service".equals("payment-service") && myEventType.equals("PAYMENT_SUCCESS")) {
                 emitEvent(timelineId, orderId, causationId, "PAYMENT_STARTED", failAt, null, executionMode, removedFaultIds, experimentSeed);
                 Thread.sleep(200);
            }
            if ("inventory-service".equals("inventory-service")) {
    ...
    private void emitEvent(String timelineId, String aggregateId, String causationId, String eventType, String failAt, String faultRuleId, String executionMode, List<String> removedFaultIds, long seed) {
        try {
            ...
            root.put("serviceName", "inventory-service");
            ...
            kafkaTemplate.send("inventory-events", outboundMessage);
